...
class AI:
    ...
    def calculateb(self, gametiles):
        ...
        # Define penalties and bonuses for specific board positions
        ...
        for x in range(8):
            for y in range(8):
                ...
                if piece in ('P', 'N', 'B', 'R', 'Q', 'K', 'p', 'n', 'b', 'r', 'q', 'k'):
                    ...
                    # penalize being on the back rank
                    for y in range(8):
                        if gametiles[y][0].pieceonTile.tostring() in piece_values:
                            value -= 20 # Penalize pieces on the back rank
                        if gametiles[y][7].pieceonTile.tostring() in piece_values:
                            value -= 20 # Penalize pieces on the back rank

                    # Add bonuses for pieces in the center of the board
                    for y in range(8):
                        for x in range(8):
                            if gametiles[y][x].pieceonTile.tostring() in active_piece_bonus:
                                value += active_piece_bonus[gametiles[y][x].pieceonTile.tostring()]

                    # Adjust the value based on board position
                    for y in range(8):
                        for x in range(8):
                            if gametiles[y][x].pieceonTile.tostring() in piece_values:
                                value += board_position_values[y][x]
        return value
